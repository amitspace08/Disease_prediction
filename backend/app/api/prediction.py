...
async def save_prediction(user_id: str, disease: str, input_data: dict, result: dict):
    try:
        ...
        print(f"Warning: Could not save prediction to MongoDB (is it running?): {e}")
...
async def predict_diabetes(input_data: DiabetesPredictionInput):
    try:
        ...
        await save_prediction("anonymous", "diabetes", data_dict, result)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.post("/heart", response_model=PredictionResponse)
async def predict_heart(input_data: HeartPredictionInput):
    try:
        data_dict = input_data.model_dump(by_alias=True)
        result = prediction_service.predict("heart", data_dict)
        await save_prediction("anonymous", "heart", data_dict, result)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.post("/liver", response_model=PredictionResponse)
async def predict_liver(input_data: LiverPredictionInput):
    try:
        data_dict = input_data.model_dump(by_alias=True)
        result = prediction_service.predict("liver", data_dict)
        await save_prediction("anonymous", "liver", data_dict, result)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@router.post("/kidney", response_model=PredictionResponse)
async def predict_kidney(input_data: KidneyPredictionInput):
    try:
        data_dict = input_data.model_dump(by_alias=True)
        result = prediction_service.predict("kidney", data_dict)
        await save_prediction("anonymous", "kidney", data_dict, result)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
